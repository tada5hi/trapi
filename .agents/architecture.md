...
**Generators must not mutate the metadata they are given** beyond the idempotent controller→method cascade. `Metadata` is routinely reused across targets (the CLI extracts once and emits every config entry from the same object), so an in-place edit in one emitter leaks into the next. V2 previously handed its operation `method.consumes` itself (`method.consumes || []` — `[]` is truthy) and pushed into it, which reached the V3 document once V3 started reading that field.
