...
- **OperationId disambiguation** — OpenAPI requires `operationId` to be unique. Under the default `'method'` strategy the first emission keeps its base id and subsequent ones get a numeric suffix (`list`, `list_2`, …), which depends on emission order. Set `swagger.data.operationIdStrategy: 'path'` to derive ids from the verb and URL instead (`getRoles`, `getRealmsByRealmIdRoles`) — see [Operation IDs](/guide/swagger-document-data#operation-ids) — or set `operationId` explicitly via your preset's metadata.
...
- **Undeclared template variables** — a path variable does not have to be a decorated argument. `@Controller(['/users', '/realms/:realmId/users'])` where middleware resolves `realmId` still emits a `required: true` string path parameter for it, because OpenAPI requires every `{variable}` in a template to be declared. Such a variable has no declaration to describe it — give it a description with [`swagger.data.pathParameters`](/guide/swagger-document-data#path-parameters). A variable you *do* decorate keeps its own type, description and validators.
